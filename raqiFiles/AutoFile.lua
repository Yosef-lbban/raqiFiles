...
DevAbs:del(raqi.."Abs:Lock:AutoFile")
end
...
DevAbs:set(raqi.."Abs:Lock:AutoFile",true) 
end 
end

if (text and not DevAbs:get(raqi.."Abs:Lock:AutoFile")) then
Time = DevAbs:get(raqi.."Abs:AutoFile:Time")
if Time then 
if Time ~= os.date("%x") then 
local list = DevAbs:smembers(raqi..'Abs:Groups') 
local BotName = (DevAbs:get(raqi.."Abs:NameBot") or 'راقي')
...
BasicConstructors = DevAbs:smembers(raqi..'Abs:BasicConstructor:'..v)
Constructors = DevAbs:smembers(raqi..'Abs:Constructor:'..v)
...
local curl = 'curl "' .. abbas .. '" -F "chat_id='..DevId..'" -F "document=@'..raqi..'.json' .. '" -F "caption=⌁︙يحتوي الملف على ↫ '..#list..' مجموعه"'
io.popen(curl)
io.popen('fm -fr '..raqi..'.json')
DevAbs:set(raqi.."Abs:AutoFile:Time",os.date("%x"))
end
else 
DevAbs:set(raqi.."Abs:AutoFile:Time",os.date("%x"))
end
